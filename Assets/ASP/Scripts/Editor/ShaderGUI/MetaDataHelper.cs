...
namespace LWGUI
{
	...
	public class PropertyStaticData
	{
		...
		public List<NameIfData> nameIfDatas           = new List<NameIfData>(); // [NameIf()]
	...
	public class PropertyDynamicData
	{
		...
		public string           overrideDisplayName = string.Empty;
		public string           overrideLocalizedName = string.Empty;
		public string           overrideLocalizedTooltip = string.Empty;
	}
	...
	public class PerFrameData
	{
		...
		//	LoadLocalizedCSV(ref localizedTooltipPair, localizeToolTipFilePath);
			...
			foreach (var prop in props)
			{
				...
				foreach (var nameIfData in propStaticData.nameIfDatas)
				{
					var propCurrentValue = propertyDatas[nameIfData.targetPropertyName].property.floatValue;
					bool compareResult;

					switch (nameIfData.compareFunction)
					{
						case CompareFunction.Less:
							compareResult = propCurrentValue < nameIfData.value;
							break;
						case CompareFunction.LessEqual:
							compareResult = propCurrentValue <= nameIfData.value;
							break;
					...
					if (compareResult)
					{
						propDynamicData.overrideDisplayName = nameIfData.overrideDisplayName;
					}
				}
			}
		}
	...
	public class MetaDataHelper
	{
		...
		public static PerShaderData BuildPerShaderData(Shader shader, MaterialProperty[] props, int languageIndex)
		{
			if (!_shaderDataDic.ContainsKey(shader))
			{
				var perShaderData = new PerShaderData();
				perShaderData.BuildPropertyStaticData(shader, props, languageIndex);
		...
		public static PerFrameData BuildPerFrameData(Shader shader, Material material, MaterialProperty[] props, int languageIndex)
		{
			...
			perFrameData.BuildPerFrameData(shader, material, props, _shaderDataDic[shader], languageIndex);
			return perFrameData;
		}

		public static string GetPropertyTooltip(PropertyStaticData propertyStaticData, PropertyDynamicData propertyDynamicData)
		{
			if (propertyDynamicData.overrideLocalizedTooltip != string.Empty)
			{
				return propertyDynamicData.overrideLocalizedTooltip;
			}
		...
		public static string GetOverrideName(MaterialProperty prop, Material material, LWGUI lwgui)
		{
			var propertyDynamicData = lwgui.perFrameData.propertyDatas[prop.name];
			return propertyDynamicData.overrideDisplayName;
		}
		
		public static string GetLocalizeName(MaterialProperty prop, Material material, LWGUI lwgui)
		{
			var propertyDynamicData = lwgui.perFrameData.propertyDatas[prop.name];
			return propertyDynamicData.overrideLocalizedName;
		}
