...
namespace LWGUI
{
	public class RampHelper
	{
			...
			out bool           doDiscard)
		{
			...
			if (GUI.Button(addRect, _iconAdd) || shouldCreate)
			{
				...
				Helper.BeginPropertyFlags = false;
			}
		...
		public static bool CreateAndSaveNewGradientTexture(int width, int height, string unityPath)
		{
			...
			gradientObject.gradient.mode = GradientMode.Fixed;
			gradientObject.gradient.colorKeys = new[] { new GradientColorKey(new Color(0.3f, 0.3f, 0.3f), 0.5f), new GradientColorKey(Color.white, 1.0f) };
		...
		public static void RampSelector(Rect rect, string rootPath, SwitchRampMapEvent switchRampMapEvent)
		{
			...
			if (e.type == UnityEngine.EventType.MouseDown && rect.Contains(e.mousePosition))
			{
				...
				var rampMaps = textureGUIDs.Select((GUID) =>
				{
					...
					{
						return new RampSelectorWindow.RampMapModel()
						{
							_rampMap = AssetDatabase.LoadAssetAtPath<Texture2D>(path),
							_rampMapFolderName = directoryInfo.Name,
						};
					}
					else
						return null;
	...
	public class RampSelectorWindow : EditorWindow
	{
		...
		public class RampMapModel
		{
			public String                   _rampMapFolderName;
			public Texture2D                   _rampMap;
		}
		
		private RampMapModel[]                   _rampMaps;
		private Vector2                       _scrollPosition;
		private RampHelper.SwitchRampMapEvent _switchRampMapEvent;
		private GUIStyle _buttonStyle;

		public static void ShowWindow(Rect rect, RampMapModel[] rampMaps, RampHelper.SwitchRampMapEvent switchRampMapEvent)
		{
			...
			window._rampMaps = rampMaps.Where(e=>e != null).ToArray();
		...
		private void OnGUI()
		{
			...
			foreach (var model in _rampMaps)
			{
				Texture2D rampMap = model._rampMap;
				EditorGUILayout.BeginHorizontal();
				if (rampMap != null)
				{
					var guiContent = new GUIContent(model._rampMapFolderName + " / " + rampMap.name);
					...
					if (GUI.Button(buttonRect, guiContent, _buttonStyle) && _switchRampMapEvent != null)
					{
